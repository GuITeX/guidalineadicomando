...
Il comando da usare è \texttt{rename} con la stessa sintassi per tutti i sistemi:
...
Compiliamo adesso un sorgente \TeX{} veramente minimo che contiene poche parole, inserendo con un editor di testi questo codice in un file, naturalmente di estensione \texttt{.tex}, che chiameremo \texttt{testtex.tex}:
\begin{verbatim}
Ciao Mondo da \TeX!
\bye
\end{verbatim}
...
Oggi questi passaggi di formato si utilizzano raramente perché si lavora direttamente e convenientemente con il formato di uscita \texttt{pdf}, come faremo tra poco con \texttt{pdflatex}.
...
Per esempio, per scontornare il file di prova precedente lasciando comunque un margine di 5 punti e sostituendo il file su disco con quello ritagliato, il comando è il seguente (per comodità utilizzate il completamento automatico premendo il tasto \keys{Tab} per produrre facilmente i nomi dei file):
\begin{verbatim}
...
\medskip
\texttt{\$ \prog{programma-di-composizione} \meta{--opzioni} \meta{nome del file}}
\medskip
